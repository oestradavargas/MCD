...
for (i in 1:nyears){
  ...
  for (j in 1:12) {
    ...
    rainfall = stack(allrasters[[ini:last]])
    
    #Calculate the total precipitation
    Total.rainfall = sum(rainfall)
    
    month_str = sprintf("%02d", j)
    
    #Rename raster files
    names(Total.rainfall)= paste0("Rtotal_",1980+i, month_str) 
    
    
    writeRaster(Total.rainfall, paste0("D:\\OneDrive - CGIAR\\CIAT\\[] MAESTRIA\\TRABAJO DE GRADO_1\\DATA\\CROPPED_DOWNSCALING\\PREC\\Rtotal_", 1980+i, month_str, ".tif"))
