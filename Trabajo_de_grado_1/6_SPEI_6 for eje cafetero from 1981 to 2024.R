...
for (i in 1:length(pixels.no.Na)){
  ...
  ##-- Calculate the SPEI-6 months (For Jul-Dec) values for each pixel --##
  spei = spei(Wat.Bal.Pixel, 6)
...
for(j in 1:nlayers(Wat.Bal)){
  ...
  date = substr(name, nchar(name) - 5, nchar(name))
  names (New_map) = paste("SPEI_6_",date, sep="")
...
for (i in 1:nlayers(SPEI_Maps.stack)){
  ...
  writeRaster(SPEI_Maps.stack[[i]], paste("D:\\OneDrive - CGIAR\\CIAT\\[] MAESTRIA\\TRABAJO DE GRADO_1\\DATA\\SPEI\\SPEI_6_months\\",name,".tif",sep =""), overwrite=TRUE)
